...
import torch
model = LexModel.load_from_checkpoint('lex_sub.ckpt').to("cuda:0")
...
def generate(
  ...
  out = model.generate(torch.ones(padded_sent_ids.shape).to("cuda:0"), padded_sent_ids.to("cuda:0"),
    torch.tensor(masked_token_id).to("cuda:0"), torch.tensor(index).to("cuda:0"))
...
with open('swords-v1.lex_sub_k_' + str(k) + '.dev.json', 'w') as f:
