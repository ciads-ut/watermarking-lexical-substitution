...
def swords_gen(model, epoch):
  ...
  def generate(
    ...
    full_padded = torch.cat([tokenized.input_ids[0,:index], torch.tensor([5, tokenized.input_ids[0,index], 5]),tokenized.input_ids[0,index + 1:]])
    start = max(1, index+1 - hparams.MAX_LENGTH//2)
    end = min(start + hparams.MAX_LENGTH, len(full_padded)-1)
    padded_sent_ids = torch.tensor([tokenizer.cls_token_id] + full_padded[start:end].tolist() + [tokenizer.sep_token_id] + (torch.zeros((hparams.MAX_LENGTH - len(full_padded[start:end].tolist())) , dtype=torch.long) + tokenizer.pad_token_id).tolist()).reshape(-1, hparams.MAX_LENGTH + 2)
  ...
  with open('swords-v1.sbert_model_swords_ep_' + str(epoch) + '.dev.json', 'w') as f:
    print()
