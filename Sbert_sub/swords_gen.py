...
model = SbertModel.load_from_checkpoint("sbert_sub.ckpt").to("cuda:1")
...
def generate(
  ...
  masked_token_id_list = tokenizer([target], add_special_tokens=False).input_ids[0]
  masked_token_id = masked_token_id_list[0]
  index = tokenized.char_to_token(len(context_1))
  full_padded = torch.cat([tokenized.input_ids[0,:index], torch.tensor([5, masked_token_id, 5]),tokenized.input_ids[0,index + len(masked_token_id_list):]])
  start = max(1, index+1 - hparams.MAX_LENGTH//2)
  end = min(start + hparams.MAX_LENGTH, len(full_padded)-1)
  padded_sent_ids = torch.tensor([[tokenizer.cls_token_id] + full_padded[start:end].tolist() + [tokenizer.sep_token_id]])
  index = hparams.MAX_LENGTH//2 if start > 1 else index
  out = model.generate(torch.ones(padded_sent_ids.shape).to("cuda:1"), padded_sent_ids.to("cuda:1"), torch.tensor(masked_token_id).to("cuda:1"), torch.tensor(index).to("cuda:1"))
  ...
  # Uncomment the following 4 lines for a sanity check of model output
  print(target)
  print(tokenizer.decode(padded_sent_ids[0]))
  print(substitutes[:10])
  print(torch.topk(out[0][0], k, dim=-1).values.tolist()[:10])
...
with open('swords-v1.sbert_sub_k_' + str(k) + '.dev.json', 'w') as f:
